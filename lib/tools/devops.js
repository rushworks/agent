...
module.exports = [
...

  {
    name: 'db_query',
    roles: ['devops'],
    description: 'Run a query against the customer database. By default, every query runs inside a READ ONLY transaction (SELECT only). If the PM has enabled db_write, write queries (INSERT / UPDATE / DELETE / DDL) are allowed. Statement timeout: 15s. Results capped at 500 rows. Connection string is held in the tool runtime and never reaches the model context.',
    parameters: {
      type: 'object',
      required: ['query'],
      properties: {
