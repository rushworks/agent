...
module.exports = [
  ...
  // ── Write (developer + devops with PM permission) ───────────────────
  {
    name: 'system_write_file',
    roles: ['developer', 'devops'],
    description: 'Create or overwrite a file with content. Creates parent directories as needed. Devops: requires file_write permission from PM; uses absolute paths; deny-list blocks sensitive paths.',
    parameters: {
      type: 'object',
    ...
    async execute({ path: p, content }, context) {
      const { working_directory, devops_config } = context;
      if (!working_directory && !(devops_config && devops_config.file_write)) {
        throw new Error('file write permission not enabled — ask the PM to enable it via the project Team tab');
      }
      const abs = resolveSafe(working_directory, p);
...

  {
    ...
    roles: ['developer', 'devops'],
    description: 'Replace an exact string in a file with new content. The old_string must match exactly once. For multiple occurrences, scope with more surrounding context. Devops: requires file_write permission from PM.',
    parameters: {
      type: 'object',
    ...
    async execute({ path: p, old_string, new_string }, context) {
      const { working_directory, devops_config } = context;
      if (!working_directory && !(devops_config && devops_config.file_write)) {
        throw new Error('file write permission not enabled — ask the PM to enable it via the project Team tab');
      }
...

  {
    name: 'system_bash',
    roles: ['developer', 'devops'],
    description: 'Run a shell command. Output is truncated at 64KB. Developer: runs in working_directory. Devops: requires shell_access permission from PM; pass `cwd` (absolute path) to set the working directory.',
    parameters: {
      type: 'object',
      required: ['command'],
      properties: {
        ...
        cwd:            { type: 'string', description: 'Absolute working directory for the command. Required for devops (no working_directory). Ignored for developer.' }
      }
    },
    async execute({ command, timeout_seconds, cwd }, context) {
      const { working_directory, devops_config } = context;
      let runDir;
      if (working_directory) {
        runDir = working_directory;
      } else {
        if (!(devops_config && devops_config.shell_access)) {
          throw new Error('shell access not enabled — ask the PM to enable it via the project Team tab');
        }
        if (!cwd || !path.isAbsolute(cwd)) {
          throw new Error('cwd (absolute path) is required for devops shell commands');
        }
        runDir = path.resolve(cwd);
      }
